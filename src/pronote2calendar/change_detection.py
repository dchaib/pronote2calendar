...
def get_changes(lessons: list[pronotepy.Lesson], events: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    add = []
    remove = []
    update = []
    ...
    for start_time, lesson_event in lesson_events.items():
        if start_time not in event_map:
            ...
            matching_events = [event for event in event_map[start_time] if (
                event.get('end', {}).get('dateTime') == lesson_event['end'].isoformat() and
                event.get('summary') == lesson_event['summary'] and
                event.get('location') == lesson_event.get('location') and
                event.get('description') == lesson_event['description']
            )]
            ...
            if matching_events:
                ...
                remove.extend([event for event in event_map[start_time] if event not in matching_events])
            else:
